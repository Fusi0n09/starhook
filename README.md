--[[
    Fusi0n Starfall - Key System
    Plans: TRIAL (24h) | WEEKLY (7d) | MONTHLY (30d) | LIFETIME
    Key format: FUSI0N-XXXX-XXXX-XXXX-PLAN
]]
...
do
    ...
    local KEY_FILE = "fusi0n_key.txt"
    local EXPIRY_FILE = "fusi0n_expiry.txt"

    -- // Owner key - bypasses everything, never expires
    local OWNER_KEY = "FUSI0N-OWNER-X9K2-M7P4-MASTER"
    ...
    local VALID_KEYS = {
        ["FUSI0N-A1B2-C3D4-E5F6-TRIAL"]    = "TRIAL",
        ["FUSI0N-G7H8-I9J0-K1L2-TRIAL"]    = "TRIAL",
        ["FUSI0N-M3N4-O5P6-Q7R8-TRIAL"]    = "TRIAL",
        ["FUSI0N-S9T0-U1V2-W3X4-TRIAL"]    = "TRIAL",
        ["FUSI0N-Y5Z6-A7B8-C9D0-TRIAL"]    = "TRIAL",
        ["FUSI0N-E1F2-G3H4-I5J6-WEEKLY"]   = "WEEKLY",
        ["FUSI0N-K7L8-M9N0-O1P2-WEEKLY"]   = "WEEKLY",
        ["FUSI0N-Q3R4-S5T6-U7V8-WEEKLY"]   = "WEEKLY",
        ["FUSI0N-W9X0-Y1Z2-A3B4-WEEKLY"]   = "WEEKLY",
        ["FUSI0N-C5D6-E7F8-G9H0-WEEKLY"]   = "WEEKLY",
        ["FUSI0N-I1J2-K3L4-M5N6-MONTHLY"]  = "MONTHLY",
        ["FUSI0N-O7P8-Q9R0-S1T2-MONTHLY"]  = "MONTHLY",
        ["FUSI0N-U3V4-W5X6-Y7Z8-MONTHLY"]  = "MONTHLY",
        ["FUSI0N-A9B0-C1D2-E3F4-MONTHLY"]  = "MONTHLY",
        ["FUSI0N-G5H6-I7J8-K9L0-MONTHLY"]  = "MONTHLY",
        ["FUSI0N-M1N2-O3P4-Q5R6-LIFETIME"] = "LIFETIME",
        ["FUSI0N-S7T8-U9V0-W1X2-LIFETIME"] = "LIFETIME",
        ["FUSI0N-Y3Z4-A5B6-C7D8-LIFETIME"] = "LIFETIME",
        ["FUSI0N-E9F0-G1H2-I3J4-LIFETIME"] = "LIFETIME",
        ["FUSI0N-K5L6-M7N8-O9P0-LIFETIME"] = "LIFETIME",
    }
    ...
    local function format_time(seconds)
        ...
        if days > 0 then return days .. "d " .. hours .. "h remaining"
        elseif hours > 0 then return hours .. "h " .. mins .. "m remaining"
        else return mins .. "m remaining" end
    end

    local screen = Instance.new("ScreenGui")
    screen.Name = "Fusi0n_KeySystem"
    ...
    box.Size = UDim2.fromOffset(400, 260)
    ...
    accent.BackgroundColor3 = Color3.fromRGB(207, 227, 0)
    accent.BorderSizePixel = 0
    accent.Parent = box
    Instance.new("UICorner", accent).CornerRadius = UDim.new(0, 8)

    local title_lbl = Instance.new("TextLabel")
    title_lbl.Size = UDim2.new(1, 0, 0, 40)
    title_lbl.Position = UDim2.fromOffset(0, 12)
    title_lbl.BackgroundTransparency = 1
    title_lbl.RichText = true
    title_lbl.Text = '<font color="rgb(207,227,0)">Fusi0n</font><font color="rgb(255,255,255)"> Starfall</font>'
    title_lbl.Font = Enum.Font.GothamBold
    title_lbl.TextSize = 22
    title_lbl.Parent = box
    ...
    local input_bg = Instance.new("Frame")
    input_bg.Size = UDim2.new(1, -40, 0, 36)
    input_bg.Position = UDim2.new(0, 20, 0, 82)
    input_bg.BackgroundColor3 = Color3.fromRGB(22, 22, 22)
    input_bg.BorderSizePixel = 0
    input_bg.Parent = box
    Instance.new("UICorner", input_bg).CornerRadius = UDim.new(0, 6)

    local input = Instance.new("TextBox")
    input.Size = UDim2.new(1, -16, 1, 0)
    input.Position = UDim2.fromOffset(8, 0)
    input.BackgroundTransparency = 1
    input.PlaceholderText = "FUSI0N-XXXX-XXXX-XXXX-PLAN"
    input.PlaceholderColor3 = Color3.fromRGB(70, 70, 70)
    input.Text = ""
    input.TextColor3 = Color3.fromRGB(207, 227, 0)
    input.Font = Enum.Font.GothamBold
    input.TextSize = 13
    input.ClearTextOnFocus = false
    input.Parent = input_bg

    local plan_badge = Instance.new("TextLabel")
    plan_badge.Size = UDim2.new(1, -40, 0, 24)
    plan_badge.Position = UDim2.new(0, 20, 0, 126)
    plan_badge.BackgroundTransparency = 1
    plan_badge.Text = ""
    plan_badge.TextColor3 = Color3.fromRGB(207, 227, 0)
    plan_badge.Font = Enum.Font.GothamBold
    plan_badge.TextSize = 12
    plan_badge.Parent = box

    local status = Instance.new("TextLabel")
    status.Size = UDim2.new(1, 0, 0, 20)
    status.Position = UDim2.new(0, 0, 0, 152)
    ...
    btn.BackgroundColor3 = Color3.fromRGB(207, 227, 0)
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(180, 200, 0)}):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(207, 227, 0)}):Play()
    end)

    local function show_expired(plan)
        for _, v in ipairs(box:GetChildren()) do v.Visible = false end
        accent.Visible = true

        local e1 = Instance.new("TextLabel")
        e1.Size = UDim2.new(1, 0, 0, 40)
        e1.Position = UDim2.fromOffset(0, 20)
        e1.BackgroundTransparency = 1
        e1.RichText = true
        e1.Text = '<font color="rgb(255,80,80)">Key Expired!</font>'
        e1.Font = Enum.Font.GothamBold
        e1.TextSize = 22
        e1.Parent = box

        local e2 = Instance.new("TextLabel")
        e2.Size = UDim2.new(1, 0, 0, 24)
        e2.Position = UDim2.fromOffset(0, 70)
        e2.BackgroundTransparency = 1
        e2.Text = "Your " .. (plan or "key") .. " plan has expired."
        e2.TextColor3 = Color3.fromRGB(180, 180, 180)
        e2.Font = Enum.Font.Gotham
        e2.TextSize = 14
        e2.Parent = box

        local e3 = Instance.new("TextLabel")
        e3.Size = UDim2.new(1, 0, 0, 20)
        e3.Position = UDim2.fromOffset(0, 100)
        e3.BackgroundTransparency = 1
        e3.Text = "Join our Discord to repurchase!"
        e3.TextColor3 = Color3.fromRGB(207, 227, 0)
        e3.Font = Enum.Font.GothamBold
        e3.TextSize = 13
        e3.Parent = box

        if isfile(KEY_FILE) then delfile(KEY_FILE) end
        if isfile(EXPIRY_FILE) then delfile(EXPIRY_FILE) end
    end

    local key_valid = false

    -- // Owner key instant bypass
    if isfile(KEY_FILE) then
        local saved_key = readfile(KEY_FILE):gsub("%s+", "")
        if saved_key == OWNER_KEY then
            plan_badge.Text = "👑 OWNER — Full Access"
            plan_badge.TextColor3 = Color3.fromRGB(207, 227, 0)
            status.TextColor3 = Color3.fromRGB(100, 255, 100)
            status.Text = "👑 Welcome back, Owner!"
            input.Text = saved_key
            _G.FUSI0N_PLAN = "OWNER"
            task.wait(1)
    ...
    if not key_valid and isfile(KEY_FILE) and isfile(EXPIRY_FILE) then
        local saved_key = readfile(KEY_FILE):gsub("%s+", "")
        local saved_data = readfile(EXPIRY_FILE):gsub("%s+", "")
        local parts = saved_data:split("|")
        local plan = parts[1]
        local expiry = tonumber(parts[2])

        if VALID_KEYS[saved_key] then
            if plan == "LIFETIME" or (expiry and get_time() < expiry) then
                local time_left = plan == "LIFETIME" and "Lifetime Access" or format_time(expiry - get_time())
                plan_badge.Text = "✅ " .. plan .. " — " .. time_left
                status.TextColor3 = Color3.fromRGB(100, 255, 100)
                status.Text = "✅ Welcome back! Loading..."
                input.Text = saved_key
                _G.FUSI0N_PLAN = plan
                task.wait(1)
                TweenService:Create(bg, TweenInfo.new(0.5), {BackgroundTransparency = 1}):Play()
    ...
    if not key_valid then
        local function try_key()
            local k = input.Text:gsub("%s+", ""):upper()
            if k == "" then
                status.TextColor3 = Color3.fromRGB(255, 80, 80)
            ...
            if k == OWNER_KEY then
                ...
                plan_badge.TextColor3 = Color3.fromRGB(207, 227, 0)
                status.TextColor3 = Color3.fromRGB(100, 255, 100)
                status.Text = "👑 Owner key accepted! Welcome!"
                btn.Active = false
                _G.FUSI0N_PLAN = "OWNER"
                task.wait(1.2)
                TweenService:Create(bg, TweenInfo.new(0.5), {BackgroundTransparency = 1}):Play()
            ...
            local plan = VALID_KEYS[k]
            if not plan then
                status.TextColor3 = Color3.fromRGB(255, 80, 80)
                status.Text = "Invalid key! Purchase at our Discord."
                plan_badge.Text = ""
                return
            end

            local duration = PLANS[plan]
            local expiry_time = plan == "LIFETIME" and 0 or (get_time() + duration)
            local time_left = plan == "LIFETIME" and "Lifetime Access" or format_time(duration)

            writefile(KEY_FILE, k)
            writefile(EXPIRY_FILE, plan .. "|" .. tostring(expiry_time))

            plan_badge.Text = "✅ " .. plan .. " — " .. time_left
            status.TextColor3 = Color3.fromRGB(100, 255, 100)
            status.Text = "Key accepted! Loading Fusi0n Starfall..."
            btn.Active = false
            _G.FUSI0N_PLAN = plan
        ...
        btn.MouseButton1Click:Connect(try_key)
        input.FocusLost:Connect(function(enter)
            if enter then try_key() end
        end)

        repeat task.wait(0.1) until key_valid
    end
end
...
do
	...
	do
		...
		function Library:Ripple(button)
			...
			ripple.BackgroundColor3 = Color3.fromRGB(207, 227, 0)
			...
			game:GetService("TweenService"):Create(ripple, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		...
		function Library:New(Properties)
			...
			UIStroke.Color = Color3.fromRGB(207, 227, 0)
			UIStroke.Thickness = 1.2
			UIStroke.Transparency = 0.4
			-- Pulsing star glow on border
			task.spawn(function()
				while UIStroke and UIStroke.Parent do
					game:GetService("TweenService"):Create(UIStroke, TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
						Transparency = 0.85
					}):Play()
					task.wait(4)
				end
			...
			Logo.Name = "Logo"
			Logo.Text = '<font color="rgb(207,227,0)">★ Starfall</font><font color="rgb(90,90,100)">  v1.0</font>'
			Logo.TextColor3 = Color3.fromRGB(207, 227, 0)
			Logo.RichText = true
			Logo.Font = Enum.Font.GothamBold
			Logo.TextSize = 15
			Logo.BackgroundTransparency = 1
			Logo.BorderSizePixel = 0
			Logo.Position = UDim2.fromOffset(5, 10)
			Logo.Size = UDim2.fromOffset(145, 30)
			Logo.TextXAlignment = Enum.TextXAlignment.Left
			Logo.Parent = Holder
			
			-- Plan badge pill next to logo
			local planColors = {
				["TRIAL"]    = {bg = Color3.fromRGB(80, 80, 80),   text = Color3.fromRGB(200, 200, 200)},
				["WEEKLY"]   = {bg = Color3.fromRGB(0, 100, 200),  text = Color3.fromRGB(180, 220, 255)},
				["MONTHLY"]  = {bg = Color3.fromRGB(120, 0, 200),  text = Color3.fromRGB(220, 180, 255)},
				["LIFETIME"] = {bg = Color3.fromRGB(207, 160, 0),  text = Color3.fromRGB(255, 240, 150)},
				["OWNER"]    = {bg = Color3.fromRGB(207, 227, 0),  text = Color3.fromRGB(0, 0, 0)},
			}
			local planLabels = {
				["TRIAL"]    = "TRIAL",
				["WEEKLY"]   = "WEEKLY",
			...
			local plan = _G.FUSI0N_PLAN or "TRIAL"
			local pColor = planColors[plan] or planColors["TRIAL"]
			local pLabel = planLabels[plan] or plan

			local Badge = Instance.new("Frame", Holder)
			Badge.Name = "PlanBadge"
			Badge.Size = UDim2.fromOffset(72, 18)
			Badge.Position = UDim2.fromOffset(6, 40)
			Badge.BackgroundColor3 = pColor.bg
			Badge.BorderSizePixel = 0
			Badge.ZIndex = 200
			local BadgeCorner = Instance.new("UICorner", Badge)
			BadgeCorner.CornerRadius = UDim.new(1, 0)
			local BadgeStroke = Instance.new("UIStroke", Badge)
			BadgeStroke.Color = pColor.text
			BadgeStroke.Thickness = 1
			BadgeStroke.Transparency = 0.5
			local BadgeLabel = Instance.new("TextLabel", Badge)
			BadgeLabel.Size = UDim2.new(1, 0, 1, 0)
			BadgeLabel.BackgroundTransparency = 1
			BadgeLabel.Text = pLabel
			BadgeLabel.TextColor3 = pColor.text
			BadgeLabel.Font = Enum.Font.GothamBold
			BadgeLabel.TextSize = 10
			BadgeLabel.ZIndex = 201

			Library.PlanBadge = Badge
			Library.PlanBadgeLabel = BadgeLabel
			
			-- Pulse glow for OWNER/LIFETIME
			if plan == "OWNER" or plan == "LIFETIME" then
				task.spawn(function()
					while Badge and Badge.Parent do
						game:GetService("TweenService"):Create(BadgeStroke, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
							Transparency = 0.8
						}):Play()
						task.wait(2.4)
					end
			...
			-- Logo bounce animation every 6 seconds
			task.spawn(function()
				while true do
					task.wait(6)
					if Logo and Logo.Parent then
						-- Bounce scale up
						game:GetService("TweenService"):Create(Logo, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
							TextSize = 19
						}):Play()
						task.wait(0.15)
						-- Scale back
						game:GetService("TweenService"):Create(Logo, TweenInfo.new(0.2, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {
							TextSize = 15
						}):Play()
						task.wait(0.2)
						-- Flash color
						game:GetService("TweenService"):Create(Logo, TweenInfo.new(0.1), {
							TextColor3 = Color3.fromRGB(255, 255, 255)
						}):Play()
						task.wait(0.15)
						game:GetService("TweenService"):Create(Logo, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
							TextColor3 = Color3.fromRGB(207, 227, 0)
						}):Play()
					else
						break
					end
			...
			-- Shimmer sweep on logo every 4 seconds
			task.spawn(function()
				-- Create shimmer frame clipped to logo area
				local shimmerClip = Instance.new("Frame")
				shimmerClip.Name = "ShimmerClip"
				shimmerClip.Size = UDim2.fromOffset(145, 30)
				shimmerClip.Position = UDim2.fromOffset(5, 10)
				shimmerClip.BackgroundTransparency = 1
				shimmerClip.BorderSizePixel = 0
				shimmerClip.ClipsDescendants = true
				shimmerClip.ZIndex = 200
				shimmerClip.Parent = Holder
				
				local shimmer = Instance.new("Frame", shimmerClip)
				shimmer.Size = UDim2.fromOffset(40, 30)
				...
				while shimmerClip and shimmerClip.Parent do
					task.wait(4)
					-- Start off left
					shimmer.Position = UDim2.fromOffset(-50, 0)
					shimmer.BackgroundTransparency = 1
					-- Sweep across
					game:GetService("TweenService"):Create(shimmer, TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
			...
			Inline.Size = UDim2.new(1,-2,1,-2)
			Inline.BackgroundColor3 = Color3.fromRGB(8, 8, 12)
			Inline.BorderSizePixel = 0
			Inline.BorderColor3 = Color3.new(0,0,0)
			Inline.ZIndex = 51
			
			-- Star dots with glow + constellation lines
			local starPositions = {}
			for s = 1, 35 do
				local sz = math.random(2, 5)
				local px = math.random(2, 98)/100
				local py = math.random(2, 98)/100
				local star = Instance.new("Frame", Holder)
				star.Size = UDim2.fromOffset(sz, sz)
				star.Position = UDim2.new(px, 0, py, 0)
				star.BackgroundColor3 = Color3.fromRGB(207, 227, 0)
				star.BackgroundTransparency = math.random(4, 8) / 10
				star.BorderSizePixel = 0
				star.ZIndex = 52
				local c = Instance.new("UICorner", star)
				c.CornerRadius = UDim.new(1, 0)
				local sg = Instance.new("UIStroke", star)
				sg.Color = Color3.fromRGB(207, 227, 0)
				sg.Thickness = math.random(1, 2)
				sg.Transparency = 0.6
				table.insert(starPositions, {x = px, y = py, frame = star})
				task.spawn(function()
					while star and star.Parent do
						task.wait(math.random(8, 25) / 10)
						local newSz = math.random(2, 6)
						game:GetService("TweenService"):Create(star, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, 0, true), {
							BackgroundTransparency = math.random(1, 6) / 10,
							Size = UDim2.fromOffset(newSz, newSz)
						}):Play()
						game:GetService("TweenService"):Create(sg, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, 0, true), {
							Transparency = math.random(2, 8) / 10
						}):Play()
					end
				end)
			end
			...
			task.wait()
			local holderSizeX = 450
			local holderSizeY = 500
			for i, starA in ipairs(starPositions) do
				for j, starB in ipairs(starPositions) do
					if j > i then
						local dx = (starA.x - starB.x) * holderSizeX
						local dy = (starA.y - starB.y) * holderSizeY
						local dist = math.sqrt(dx*dx + dy*dy)
						-- Only connect stars that are close enough
						if dist < 100 and dist > 20 then
							local line = Instance.new("Frame", Holder)
							line.BackgroundColor3 = Color3.fromRGB(207, 227, 0)
							line.BackgroundTransparency = 0.85
							line.BorderSizePixel = 0
							line.ZIndex = 51
							line.AnchorPoint = Vector2.new(0, 0.5)
							-- Position at star A
							local midX = (starA.x + starB.x) / 2
							local midY = (starA.y + starB.y) / 2
							line.Size = UDim2.fromOffset(dist, 1)
							line.Position = UDim2.new(starA.x, 0, starA.y, 0)
							-- Rotate toward star B
							local angle = math.deg(math.atan2(dy, dx))
							line.Rotation = angle
							-- Fade in slowly
							game:GetService("TweenService"):Create(line, TweenInfo.new(2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			...
			-- // Server Info Bar (bottom of sidebar)
			local ServerBar = Instance.new("Frame", Inline)
			ServerBar.Name = "ServerBar"
			ServerBar.Size = UDim2.new(0, 148, 0, 44)
			ServerBar.Position = UDim2.new(0, 1, 1, -45)
			ServerBar.BackgroundColor3 = Color3.fromRGB(10, 10, 15)
			ServerBar.BorderSizePixel = 0
			ServerBar.ZIndex = 55
			local SBC = Instance.new("UICorner", ServerBar)
			SBC.CornerRadius = UDim.new(0, 6)
			local SBStroke = Instance.new("UIStroke", ServerBar)
			SBStroke.Color = Color3.fromRGB(207, 227, 0)
			SBStroke.Thickness = 1
			SBStroke.Transparency = 0.7
			
			-- Top row: ping + players
			local SBRow1 = Instance.new("TextLabel", ServerBar)
			SBRow1.Size = UDim2.new(1, -8, 0, 18)
			SBRow1.Position = UDim2.fromOffset(4, 4)
			SBRow1.BackgroundTransparency = 1
			SBRow1.TextColor3 = Color3.fromRGB(180, 180, 180)
			SBRow1.Font = Enum.Font.GothamBold
			SBRow1.TextSize = 11
			SBRow1.TextXAlignment = Enum.TextXAlignment.Left
			SBRow1.Text = "📡 Ping: -- ms  👥 --/--"
			SBRow1.ZIndex = 56
			SBRow1.BorderSizePixel = 0
			
			-- Bottom row: region + server age
			local SBRow2 = Instance.new("TextLabel", ServerBar)
			SBRow2.Size = UDim2.new(1, -8, 0, 16)
			SBRow2.Position = UDim2.fromOffset(4, 24)
			SBRow2.BackgroundTransparency = 1
			SBRow2.TextColor3 = Color3.fromRGB(120, 120, 130)
			SBRow2.Font = Enum.Font.Gotham
			SBRow2.TextSize = 10
			SBRow2.TextXAlignment = Enum.TextXAlignment.Left
			SBRow2.Text = "🌍 Region: --  ⏱ --"
			SBRow2.ZIndex = 56
			SBRow2.BorderSizePixel = 0
			...
			task.spawn(function()
				...
				while ServerBar and ServerBar.Parent do
					-- Ping
					local ping = "?"
					pcall(function()
					...
					-- Server age
					local elapsed = tick() - serverStart
					local mins = math.floor(elapsed / 60)
					local secs = math.floor(elapsed % 60)
					local ageStr = string.format("%dm %ds", mins, secs)
					
					-- Ping color
					local pingColor
					if type(ping) == "number" then
						if ping < 80 then
							pingColor = Color3.fromRGB(100, 255, 100)
						elseif ping < 150 then
							pingColor = Color3.fromRGB(255, 210, 50)
						else
							pingColor = Color3.fromRGB(255, 80, 80)
						end
						SBRow1.Text = "📡 " .. ping .. "ms  👥 " .. playerCount .. "/" .. maxPlayers
					else
						pingColor = Color3.fromRGB(180, 180, 180)
						SBRow1.Text = "📡 --ms  👥 " .. playerCount .. "/" .. maxPlayers
					end
					SBRow1.TextColor3 = pingColor
					SBRow2.Text = "🌍 Job: " .. string.sub(game.JobId, 1, 6) .. "...  ⏱ " .. ageStr
			...
			local holderInline = Library.Holder:FindFirstChild("Inline")
			if holderInline then holderInline.BackgroundTransparency = 1 end
			Library.Holder.Size = UDim2.new(0, Window.Size.X.Offset, 0, 0)
			game:GetService("TweenService"):Create(Library.Holder, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {
				Size = UDim2.new(0, Window.Size.X.Offset, 0, Window.Size.Y.Offset),
				BackgroundTransparency = 0
			}):Play()
			task.spawn(function()
				task.wait(0.2)
				if holderInline then
					game:GetService("TweenService"):Create(holderInline, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {BackgroundTransparency = 0}):Play()
				end
			end)
		...
		function Library:Page(Properties)
			...
			-- Glow under tab
			local TabGlow = Instance.new("Frame", TabButton)
			TabGlow.Name = "TabGlow"
			TabGlow.Size = UDim2.new(0.7, 0, 0, 4)
			TabGlow.AnchorPoint = Vector2.new(0.5, 1)
			TabGlow.Position = UDim2.new(0.5, 0, 1, 2)
			TabGlow.BackgroundColor3 = Library.Accent
			TabGlow.BorderSizePixel = 0
			TabGlow.BackgroundTransparency = 1
			TabGlow.ZIndex = 60
			local GlowCorner = Instance.new("UICorner", TabGlow)
			GlowCorner.CornerRadius = UDim.new(1, 0)
			local GlowStroke = Instance.new("UIStroke", TabGlow)
			GlowStroke.Color = Library.Accent
			GlowStroke.Thickness = 3
			GlowStroke.Transparency = 1
			table.insert(Library.ThemeObjects, TabGlow)
			table.insert(Library.ThemeObjects, GlowStroke)
			...
			local TabGrad = Instance.new("UIGradient", TabButton)
			...
			TabStroke.Color = Library.Accent
			TabStroke.Thickness = 1
			TabStroke.Transparency = 1
			table.insert(Library.ThemeObjects, TabStroke)
			...
			Title.TextColor3 = Color3.fromRGB(100, 100, 115)
			...
			Icon.ImageColor3 = Color3.fromRGB(100, 100, 115)
			
			-- Hover effect
			TabButton.MouseEnter:Connect(function()
				if not Page.Open then
					game:GetService("TweenService"):Create(TabButton, TweenInfo.new(0.15), {BackgroundTransparency = 0.6}):Play()
					game:GetService("TweenService"):Create(Title, TweenInfo.new(0.15), {TextColor3 = Color3.fromRGB(160, 160, 175)}):Play()
					game:GetService("TweenService"):Create(Icon, TweenInfo.new(0.15), {ImageColor3 = Color3.fromRGB(160, 160, 175)}):Play()
					TabGrad.Enabled = true
				end
			end)
			TabButton.MouseLeave:Connect(function()
				if not Page.Open then
					game:GetService("TweenService"):Create(TabButton, TweenInfo.new(0.15), {BackgroundTransparency = 1}):Play()
					game:GetService("TweenService"):Create(Title, TweenInfo.new(0.15), {TextColor3 = Color3.fromRGB(100, 100, 115)}):Play()
					game:GetService("TweenService"):Create(Icon, TweenInfo.new(0.15), {ImageColor3 = Color3.fromRGB(100, 100, 115)}):Play()
					TabGrad.Enabled = false
				end
			end)
			--
			...
			function Page:Turn(bool)
				...
				task.spawn(function()
					...
					-- Slide in elements when tab opens
					if Page.Open then
						local allElements = {}
						...
						for i, element in ipairs(allElements) do
							task.spawn(function()
								...
								if element and element.Parent then
									...
									element.Position = UDim2.new(origPos.X.Scale, origPos.X.Offset - 20, origPos.Y.Scale, origPos.Y.Offset)
									element.BackgroundTransparency = 1
									game:GetService("TweenService"):Create(element, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
										Position = origPos,
										BackgroundTransparency = element:GetAttribute("OrigTransparency") or 0
									}):Play()
									for _, child in ipairs(element:GetDescendants()) do
										if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then
											game:GetService("TweenService"):Create(child, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
												TextTransparency = 0
											}):Play()
										end
									end
				...
				local activeColor = Library.Accent
				local inactiveColor = Color3.fromRGB(100, 100, 115)
				game:GetService("TweenService"):Create(TabButton, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = Page.Open and 0 or 1}):Play()
				game:GetService("TweenService"):Create(Title, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = Page.Open and activeColor or inactiveColor}):Play()
				game:GetService("TweenService"):Create(Icon, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Page.Open and activeColor or inactiveColor}):Play()
				game:GetService("TweenService"):Create(Accent, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = Page.Open and 0 or 1}):Play()
				-- Glow stroke on active
				local stroke = TabButton:FindFirstChildOfClass("UIStroke")
				if stroke then
					game:GetService("TweenService"):Create(stroke, TweenInfo.new(0.2), {Transparency = Page.Open and 0.6 or 1}):Play()
				end
				local grad = TabButton:FindFirstChildOfClass("UIGradient")
				if grad then grad.Enabled = Page.Open end
				
				-- Glow under tab animate
				local tabGlow = TabButton:FindFirstChild("TabGlow")
				if tabGlow then
					game:GetService("TweenService"):Create(tabGlow, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
						BackgroundTransparency = Page.Open and 0.3 or 1
					}):Play()
					local glowStroke = tabGlow:FindFirstChildOfClass("UIStroke")
					if glowStroke then
						game:GetService("TweenService"):Create(glowStroke, TweenInfo.new(0.25), {
							Transparency = Page.Open and 0.7 or 1
						}):Play()
					end
					-- Pulse when first activated
					if Page.Open then
						task.spawn(function()
							game:GetService("TweenService"):Create(tabGlow, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, true), {
		...
		function Sections:Toggle(Properties)
			...
			-- // Returning
			Library:Connection(NewToggle.MouseButton1Click, function()
				Library:Ripple(NewToggle)
				SetState()
			end)
			return Toggle
		end
		...
		function Sections:Slider(Properties)
			...
			Library:Connection(NewSlider.InputBegan, function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					...
					Slide(input)
					-- Wave pulse on click
					game:GetService("TweenService"):Create(Fill, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, true), {
						BackgroundTransparency = 0.4
					}):Play()
					game:GetService("TweenService"):Create(Circle, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			...
			Library:Connection(NewSlider.InputEnded, function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					Sliding = false
					-- Shrink circle back
					game:GetService("TweenService"):Create(Circle, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "Fusi0n Starfall | placeholder");
				AnimateText = nil;
			}
			--
			local Outline = Instance.new("Frame")
			...
			task.spawn(function()
				while task.wait() do
					for i = 1, #"Fusi0n Starfall" do
						Watermark.AnimateText = string.sub("Fusi0n Starfall", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;

					for i = #"Fusi0n Starfall.club" - 1, 1, -1 do
						Watermark.AnimateText = string.sub("Fusi0n Starfall", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;
...
if (not (isfolder("Fusi0n Starfall"))) then
	makefolder("Fusi0n Starfall");
end;
...
screen_gui.Name = "Fusi0n Starfall";
...
screen_gui_2.Name = "Fusi0n Starfall";
...
local silent_aim = {} do
	silent_aim.get_predicted_position = function()
		...
		--// end
		return game.PlaceId == hood_customs and predicted_position + Vector3.new(0, 0, 0) or predicted_position
	end;
end;
...
do
	...
	do
		...
		instances["local_text"] = utility.instance_new("TextLabel", { 
			Name = "Fusi0n Starfall",
			Parent = screen_gui_2,
			Text = '<font color="rgb(207, 227, 0)">Fusi0n Starfall</font><font color="rgb(255, 255, 255)">.club</font>',
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
...
do
    ...
    do
        utility.new_connection(run_service.RenderStepped, function()
            ...
            if flags["legit_silent_live_prediction"] then
                locals.prediction_adjuster = locals.prediction_adjuster or {last_update = 0};
                
                if tick() - locals.prediction_adjuster.last_update > 2 then
                    locals.prediction_adjuster.last_update = tick();
                    ...
                    local base = 0.134;
                    local offset = (ping - 50) / 1000;
                    local new_pred = math.clamp(base + offset, 0.08, 0.25);
                    new_pred = math.floor(new_pred * 1000) / 1000;
                    
                    Library.Flags["legit_silent_prediction"] = tostring(new_pred);
                end;
            end;
            
            -- FOV circle visual is independent from targeting
            -- silent aim works with or without FOV enabled
            local effective_fov = fov_enabled;

            if (silent_enabled) then
                local new_target = combat.get_closest_player(effective_fov, fov_radius, checks_enabled, check_values);
            ...
            if (silent_enabled and fov_visualize_enabled) then
                --// settings
            ...
            if (silent_enabled and (tracer_enabled) and (locals.silent_aim.is_targetting and locals.silent_aim.target)) then
                local mouse_position = user_input_service:GetMouseLocation();
                local predicted_position = utility.world_to_screen(game.PlaceId == hood_customs and locals.silent_aim.predicted_position or locals.silent_aim.predicted_position);
        ...
        do
            utility.new_connection(run_service.Heartbeat, function()
				...
				if (target_aim_enabled and (visuals_enabled and dot_enabled) and (is_targetting and target) and utility.has_character(target)) then
                    local predicted_position = utility.world_to_screen(game.PlaceId == hood_customs and locals.target_aim.predicted_position or locals.target_aim.predicted_position);
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if (enabled) then
					...
					local text = custom_text_enabled and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>' or '<font color="' .. text_color .. '">Fusi0n Starfall</font><font color="rgb(255, 255, 255)">.club</font>';
            ...
            utility.new_connection(character.ChildAdded, function(object)
                ...
                if object == gun.tool then
					connections.gun["activated"] = utility.new_connection(gun.tool.Activated, function()
                        ...
                        -- Humanizer helper
                        local function humanized_pos(pos)
                            if flags["legit_silent_humanizer"] then
                                local s = flags["legit_silent_humanizer_strength"] or 0.03
                        ...
                        if (flags["legit_silent_enabled"] and flags["legit_silent_anti_aim_viewer"] and (locals.silent_aim.is_targetting and locals.silent_aim.target)) then
                            remote:FireServer(mouse_argument, humanized_pos(locals.silent_aim.predicted_position));
                        end;
...
do
	...
	local OWNER_USERNAME = "dv_bdw"
	
	local function notifyOwnerJoin(player)
		-- Get profile picture
		local headshot = ""
		pcall(function()
			headshot = Players:GetUserThumbnailAsync(
				player.UserId,
		...
		pcall(function()
			StarterGui:SetCore("SendNotification", {
				Title = "👑 Fusi0n Starfall",
				Text = "⭐ Starfall developer has joined the server!",
				Icon = headshot,
				Duration = 15
			})
		end)
		
		-- Big on screen announcement
		local announcement = Instance.new("Frame")
		announcement.Size = UDim2.new(0, 340, 0, 70)
		announcement.Position = UDim2.new(0.5, -170, 0, -80)
		announcement.BackgroundColor3 = Color3.fromRGB(8, 8, 12)
		announcement.BorderSizePixel = 0
		announcement.ZIndex = 999
		announcement.Parent = Library.ScreenGUI
		
		local ac = Instance.new("UICorner", announcement)
		ac.CornerRadius = UDim.new(0, 10)
		
		local astroke = Instance.new("UIStroke", announcement)
		astroke.Color = Color3.fromRGB(207, 227, 0)
		astroke.Thickness = 1.5
		astroke.Transparency = 0.3
		
		-- Gradient background
		local agrad = Instance.new("UIGradient", announcement)
		agrad.Color = ColorSequence.new{
			ColorSequenceKeypoint.new(0, Color3.fromRGB(20, 20, 10)),
			ColorSequenceKeypoint.new(1, Color3.fromRGB(8, 8, 12))
		}
		agrad.Rotation = 90
		
		-- Crown icon
		local crown = Instance.new("TextLabel", announcement)
		crown.Size = UDim2.fromOffset(50, 70)
		crown.Position = UDim2.fromOffset(10, 0)
		...
		-- Title
		local atitle = Instance.new("TextLabel", announcement)
		atitle.Size = UDim2.new(1, -70, 0, 30)
		atitle.Position = UDim2.fromOffset(60, 8)
		atitle.BackgroundTransparency = 1
		atitle.Text = "Fusi0n Starfall"
		atitle.TextColor3 = Color3.fromRGB(207, 227, 0)
		atitle.Font = Enum.Font.GothamBold
		atitle.TextSize = 16
		atitle.TextXAlignment = Enum.TextXAlignment.Left
		atitle.ZIndex = 1000
		atitle.BorderSizePixel = 0
		
		-- Message
		local amsg = Instance.new("TextLabel", announcement)
		amsg.Size = UDim2.new(1, -70, 0, 24)
		amsg.Position = UDim2.fromOffset(60, 36)
		amsg.BackgroundTransparency = 1
		amsg.Text = "⭐ Starfall developer has joined the server!"
		amsg.TextColor3 = Color3.fromRGB(200, 200, 200)
		amsg.Font = Enum.Font.Gotham
		amsg.TextSize = 12
		amsg.TextXAlignment = Enum.TextXAlignment.Left
		amsg.ZIndex = 1000
		amsg.BorderSizePixel = 0
		
		-- Slide down in
		game:GetService("TweenService"):Create(announcement, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
		...
		task.spawn(function()
			for i = 1, 3 do
				game:GetService("TweenService"):Create(astroke, TweenInfo.new(0.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, 0, true), {
					Transparency = 0.8
				}):Play()
				task.wait(0.8)
			end
		...
		task.delay(6, function()
			game:GetService("TweenService"):Create(announcement, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
	...
	task.spawn(function()
		...
		for _, player in ipairs(Players:GetPlayers()) do
			if player.Name:lower() == OWNER_USERNAME:lower() and player ~= Players.LocalPlayer then
				notifyOwnerJoin(player)
				break
			end
		end
		
		-- Watch for owner joining
		Players.PlayerAdded:Connect(function(player)
			if player.Name:lower() == OWNER_USERNAME:lower() then
				task.wait(1)
				notifyOwnerJoin(player)
			end
		end)
	...
	-- // Mod / Admin Detector (from vf.lua)

	-- Group IDs to detect (add more as needed)
	local MOD_GROUPS = {
		693335996,  -- default from vf.lua
		-- add more group IDs here
	}

	-- Known staff/mod usernames (extra layer)
	local MOD_NAMES = {
		"Roblox", "HelperBot", "ModerationAux"
	}
	...
	local function isStaff(player)
		-- Check groups
		for _, groupId in ipairs(MOD_GROUPS) do
	...
	local function alertMod(player)
		local headshot = getHeadshot(player.UserId)

		-- Roblox notification popup
		pcall(function()
			StarterGui:SetCore("SendNotification", {
				Title = "⚠️ MOD DETECTED",
				Text = player.Name .. " (" .. player.DisplayName .. ") is in this server!",
				Icon = headshot,
				Duration = 30
			})
		end)

		-- Also show Fusi0n notification
		Library:Notification("⚠️ MOD IN SERVER: " .. player.Name, 10)
	end

	-- Scan players already in server on load
	task.spawn(function()
		task.wait(2) -- wait for script to fully load
		for _, player in ipairs(Players:GetPlayers()) do
	...
	Library.ModGroups = MOD_GROUPS

	-- // Discord Webhook on Join
	...
	task.spawn(function()
		...
		local payload = {
			embeds = {
				{
					title = "⭐ Fusi0n Starfall — Player Joined",
					color = 13664256, -- yellow #D0E300
					thumbnail = {
					...
					fields = {
						...
						{name = "🌐 Server", value = "[Join](https://www.roblox.com/games/" .. placeId .. "?gameInstanceId=" .. serverId .. ")", inline = true},
						{name = "👥 Players", value = playerCount .. "/" .. maxPlayers, inline = true},
						{name = "📡 Ping", value = ping, inline = true},
					},
					footer = {
						text = "Fusi0n Starfall v1.0 • fusi0n.gg"
					},
		...
		if success then
			Library:Notification("📡 Webhook sent successfully!", 4)
		end
	end)
	...
	FPSLabel.Size = UDim2.fromOffset(90, 28)
	FPSLabel.Position = UDim2.new(1, -100, 0, 8)
	...
	FPSLabel.ZIndex = 100
	FPSLabel.Parent = Library.ScreenGUI
	local _fc = Instance.new("UICorner", FPSLabel)
	_fc.CornerRadius = UDim.new(0, 6)
	local _fs = Instance.new("UIStroke", FPSLabel)
	_fs.Color = Color3.fromRGB(100, 255, 100)
	_fs.Thickness = 1
	_fs.Transparency = 0.5
	Library.FPSLabel = FPSLabel
	task.spawn(function()
			...
			if tick() - last >= 1 then
				...
				if FPSLabel.Visible then
					...
					if fps >= 55 then
						...
						_fs.Color = Color3.fromRGB(100, 255, 100)
					elseif fps >= 30 then
						FPSLabel.TextColor3 = Color3.fromRGB(255, 210, 50)
						_fs.Color = Color3.fromRGB(255, 210, 50)
					else
						FPSLabel.TextColor3 = Color3.fromRGB(255, 80, 80)
						_fs.Color = Color3.fromRGB(255, 80, 80)
					end
				end
			end
		end)
	...
	do
		...
		do
			...
			do
				...
				local fov_option_list = fov_toggle:OptionList({});
				fov_option_list:Toggle({Name = "Show FOV Circle", Flag = "legit_silent_visualize_field_of_view"});
				fov_option_list:Colorpicker({Name = "Circle Color", Flag = "legit_silent_field_of_view_color", Default = default_color});
				fov_option_list:Slider({Name = "Circle Transparency", Flag = "legit_silent_field_of_view_transparency", Default = 0, Minimum = 0, Maximum = 1, Decimals = 0.01, Ending = "%"});
				fov_option_list:Slider({Name = "FOV Radius", Flag = "legit_silent_field_of_view_radius", Default = 1, Minimum = 1, Maximum = 200, Decimals = 0.01, Ending = "%"});
				-- Note: Silent aim works even with FOV disabled
				
				local humanizer_toggle = legit_silent_aim:Toggle({Name = "Humanizer", Flag = "legit_silent_humanizer"});
				local humanizer_list = humanizer_toggle:OptionList({});
				humanizer_list:Slider({Name = "Strength", Flag = "legit_silent_humanizer_strength", Default = 0.03, Minimum = 0.01, Maximum = 0.15, Decimals = 0.001, Ending = ""});
				humanizer_list:Slider({Name = "Max Delay (ms)", Flag = "legit_silent_humanizer_delay", Default = 35, Minimum = 0, Maximum = 100, Decimals = 1, Ending = "ms"});
				...
				local auto_pred_toggle = legit_silent_aim:Toggle({Name = "Auto Prediction", Flag = "legit_silent_auto_prediction"});
				local auto_pred_list = auto_pred_toggle:OptionList({});
				auto_pred_list:Button({Name = "🎯 Calibrate Now", Callback = function()
					Library:Notification("🎯 Calibrating prediction...", 3)
					
					task.spawn(function()
						-- Auto prediction calibration logic
						local Stats = game:GetService("Stats")
						local ping = 0
						...
						-- Base prediction formula based on ping
						local base_pred = 0.134
						local ping_offset = (ping - 50) / 1000
						local calculated = base_pred + ping_offset
						
						-- Clamp between reasonable values
						calculated = math.clamp(calculated, 0.08, 0.25)
						
						-- Round to 3 decimals
						calculated = math.floor(calculated * 1000) / 1000
						
						-- Set the prediction
						Library.Flags["legit_silent_prediction"] = tostring(calculated)
						
						Library:Notification("✅ Prediction set to " .. calculated .. " (Ping: " .. math.floor(ping) .. "ms)", 5)
					end)
				end});
				auto_pred_list:Toggle({Name = "Live Adjust", Flag = "legit_silent_live_prediction"});
				legit_silent_aim:Button({Name = "Auto-Tune Prediction", Callback = function()
					Library:Notification("🔮 Auto-tuning prediction...", 3)
					
					task.spawn(function()
						-- Test different prediction values
						local test_values = {0.120, 0.125, 0.130, 0.134, 0.138, 0.142, 0.146, 0.150}
						local best_value = 0.134
						local best_score = 0
						
						for i, pred_val in ipairs(test_values) do
							Library.Flags["legit_silent_prediction"] = tostring(pred_val)
							task.wait(0.5)
							
							-- Simulate testing (in real scenario, track hits)
						...
						-- Set best prediction
						Library.Flags["legit_silent_prediction"] = tostring(best_value)
						Library:Notification("✅ Best prediction: " .. best_value, 5)
					end)
				end});
			end;
	...
	do
		...
		local starhook_classics = ui.tabs["anti_aim"]:Section({Name = "Fusi0n Starfall Classics", Side = "Left", Size = 150});
	...
	do --// credits to finobe wtv im way too lazy
		...
		if not isfolder("Fusi0n Starfall/configs") then 
		    makefolder("Fusi0n Starfall/configs");
		end;

		local function update_config_list()
		    local list = {};
		
		    for idx, file in listfiles("Fusi0n Starfall/configs") do
		        local file_name = file:gsub("Fusi0n Starfall/configs\\", ""):gsub(".cfg", ""):gsub("Fusi0n Starfall/configs/", "");
		...
		cfgs:Button({Name = "Create", Callback = function()
		    ...
		    if config_name == "" or isfile("Fusi0n Starfall/configs/" .. config_name .. ".cfg") then
		        return;
		    end;
		    writefile("Fusi0n Starfall/configs/" .. config_name .. ".cfg", Library:GetConfig());
		    update_config_list();
		end});
		...
		cfgs:Button({Name = "Save", Callback = function()
		    ...
		    if selected_config then
		        writefile("Fusi0n Starfall/configs/" .. selected_config .. ".cfg", Library:GetConfig());
		    end;
		end});
		...
		cfgs:Button({Name = "Load", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        Library:LoadConfig(readfile("Fusi0n Starfall/configs/" .. selected_config .. ".cfg"));
		    end;
		end});
		...
		cfgs:Button({Name = "Delete", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        delfile("Fusi0n Starfall/configs/" .. selected_config .. ".cfg");
		    end;
		    update_config_list();
		end});
		...
		window:Toggle({Name = "FPS Counter", Flag = "ui_fps", Callback = function(state)
			if Library.FPSLabel then
				Library.FPSLabel.Visible = state
			end
		end});

		window:Textbox({Name = "Webhook URL", Flag = "webhook_url", Placeholder = "https://discord.com/api/webhooks/...", Callback = function(text)
			WEBHOOK_URL = text
		end});
		
		window:Toggle({Name = "Webhook on Join", Flag = "webhook_enabled", Default = true, Callback = function(state)
			Library.WebhookEnabled = state
		end});

		window:Toggle({Name = "Mod Detector", Flag = "mod_detector", Default = true, Callback = function(state)
			Library.ModDetectorEnabled = state
		end});

		window:Button({Name = "🌍 Server Hop", Callback = function()
			local TeleportService = game:GetService("TeleportService")
			local Players = game:GetService("Players")
			local placeId = game.PlaceId
			
			Library:Notification("🌍 Finding new server...", 3)
			...
			task.spawn(function()
				...
				if success and servers and servers.data then
					for _, server in ipairs(servers.data) do
						if server.id ~= game.JobId and server.playing < server.maxPlayers then
							...
							if ok then
								Library:Notification("✅ Teleporting to new server!", 2)
								return
							end
						end
					end
				...
				-- Fallback: just teleport to same place
				pcall(function()
		...
		window:Textbox({Name = "Add Mod Group ID", Flag = "mod_group_id", Placeholder = "Enter Roblox Group ID", Callback = function(text)
			local id = tonumber(text)
			if id then
				table.insert(Library.ModGroups, id)
				Library:Notification("✅ Group " .. id .. " added to mod list!", 4)
			end
		end});

		window:List({Name = "Color Theme", Flag = "ui_color_theme", Options = {"Starfall Yellow", "Ocean Blue", "Royal Purple", "Blood Red", "Emerald Green", "Snow White"}, Default = "Starfall Yellow", Callback = function(theme)
			local themes = {
				["Starfall Yellow"] = Color3.fromRGB(207, 227, 0),
				["Ocean Blue"]      = Color3.fromRGB(0, 180, 255),
				["Royal Purple"]    = Color3.fromRGB(180, 80, 255),
				["Blood Red"]       = Color3.fromRGB(255, 50, 50),
				["Emerald Green"]   = Color3.fromRGB(0, 220, 120),
				["Snow White"]      = Color3.fromRGB(230, 230, 240),
			}
